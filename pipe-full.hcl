...
#
# I implemented leaq but in a different form such that unlike the seq version, its description is leaq C(rA), rB. Thus, I changed the places of rA and rB. It is not a major difference but
# destination register is rB in this implementation. Its implementation is just like in seq, however because of the place difference of rA and rB, I added 1 more line to Execute Stage
# word_aluB such that E_icode in { ILEAQ } : E_valA. For input A, its valC and for input B, its valA. Also just in case, leaq set condition codes in this implementation but probably I had never used it.
...
word d_srcA = [
	D_icode in { IRRMOVQ, IRMMOVQ, IOPQ, IPUSHQ, IICMPQ, ILEAQ  } : D_rA;
...
word d_dstE = [
	D_icode in { IRRMOVQ, IIRMOVQ, IOPQ, ILEAQ } : D_rB;
	D_icode in { IPUSHQ, IPOPQ, ICALL, IRET } : RRSP;
...
## Should the condition codes be updated?
	bool set_cc = E_icode in { IOPQ, IICMPQ, ILEAQ } &&
